 ...
 {showModal ? (
  ...
  activeShift.isWorking ? (
    <div className='flex flex-col w-full'>
      <h1 className='text-black'> {activeShift.Name} are working</h1>
      <h1 className='text-black'> {activeShift.Name} email</h1>
      <button onClick={()=> updateEmployeeWork(activeShift.id)} className='bg-emerald-800 text-white font-bold py-2 px-4 rounded-full'>end his shift </button>
    </div>
  ) : Object.keys(activeShift).length === 0 ? (
    <div className='flex flex-col'>
      <h1 className='text-black'>No one is working right now</h1>
    </div>
  ) : (
    <div className='flex flex-col'>
